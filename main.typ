...
// This chapter details the methodology for augmenting Adversarial Motion Priors with a discrete latent representation of motion. The exposition begins formally defining the research hypothesis and an overview of the conditioned control formulation, then proceeds through the five stages of the workflow: data retargeting and feature extraction (Section 4.2), discrete latent structuring with a motion VQ-VAE (Section 4.3), the baseline reinforcement learning framework and its extension to MuJoCo (Section 4.4), the latent-conditioned AMP training that constitutes the principal contribution (Section 4.5), and the interactive system that integrates the trained policy into a game engine (Section 4.6). Section 4.7 states the evaluation protocol, and Section 4.8 sets out the research plan, distinguishing the milestone reached at proposal time from the principal contribution executed over the remainder of the thesis.

== Overview

This research is driven by the hypothesis that augmenting AMP with discrete motion tokens from a VQ-VAE can effectively mitigate both mode-averaging and mode-collapse problems. By explicitly structuring the reference data, this discrete representation resolves the ambiguities that traditionally affects adversarial imitation learning when controlling characters across diverse motion datasets. AMP removes the need for manual reward engineering by replacing the handcrafted imitation reward with a learned discriminator that scores motion naturalness, but it provides no explicit mechanism to index and request distinct behaviors from a heterogeneous motion library. By first structuring the reference motion space through a discrete information bottleneck, such as a finite codebook of learned prototype vectors, and then supplying the resulting tokens as an additional conditioning input to the policy, the framework gains an explicit skill-selection channel while retaining AMP's data-driven naturalness.

Concretely, the control policy is conditioned not only on the features of the proprioceptive state $Phi(s_t)$ and the task goal $g_t$, as in vanilla AMP, but additionally on a discrete latent code $k$ drawn from a learned motion codebook:

$ a_t = pi_theta (s_t, g_t, k) $

The code $k$ specifies which gait the character should perform, while the goal $g_t$ continues to specify the desired heading direction and speed. The AMP discriminator $D_psi (Phi(s_t), Phi(s_(t+1)))$ continues to supply the style reward $r^S_t$, and the total reward retains the form used in imitation learning frameworks discussed previously, so that the latent code adds a control channel without disturbing the reward structure that makes AMP data driven approach scalable. The codebook itself is produced by a motion VQ-VAE trained on the retargeted reference data, described in Section 4.3.

To investigate this hypothesis systematically, the methodology is organized as a five-stage workflow: (i) data acquisition and preprocessing through motion retargeting; (ii) motion data discrete latent structuring; (iii) baseline framework implementation; (iv) latent-conditioned policy training; and (v) system integration into an interactive application.

== Motion Data Retargeting

The reference motion data that serves as the style objective for the policy is the quadruped motion capture dataset used in Mode-Adaptive Neural Networks (MANN) framework for quadruped motion control @zhang_mode-adaptive_2018, which records a real dog performing a diverse repertoire of gaits across roughly sixty skeletal joints.

// ADD PICTURE OR TABLE OF GAITS IN MANN DATASET

Because the morphology of the MANN dog skeleton differs from the target platform, the motion clips must be retargeted to the Unitree Go2. The retargeting follows the inverse-kinematics approach presented in @bin_peng_learning_2020, who mapped dog motion capture to a quadruped robot (Laikago/A1) by aligning corresponding body parts and solving for joint configurations that reproduce the reference end-effector and root trajectories. Applying an analogous explicit IK retargeting, the MANN skeleton is mapped to the Go2 skeleton, yielding Go2 joint-angle trajectories aligned frame-by-frame to the reference clips. Once retargeted, the reference data and the simulated Go2 share a skeletal structure, so the discriminator features and the codebook input features can be extracted from both sources without any further morphological transfer.

// ADD PICTURE HERE OF RETARGETED KEY-POINTS IN EACH SKELETON

The retargeted motion is then reduced to a kinematic feature representation. Each frame is mapped to a feature vector $Phi(s_t)$ comprising root height and orientation, body linear and angular velocities, joint positions relative to the root, joint velocities, and foot-contact states. The resulting consecutive-frame features $(Phi(s_t), Phi(s_(t+1)))$ constitute the transition pairs on which the discriminator is trained, and the windowed feature time series provides the input to the VQ-VAE encoder.

// Generated artifacts. (1) The retargeted quadruped motion dataset, compatible with the Unitree Go2 morphology. (2) A preprocessed .npy time series of the extracted features $\big(\Phi(s_t), \Phi(s_{t+1})\big)$, parsed from the source BVH.

== Discrete Latent Space Structuring <method:discrete>

The second stage imposes a queryable structure on the retargeted dataset by training a motion VQ-VAE comprising an encoder $E$, a discrete codebook $cal(C) = {e_1, ..., e_K}$, and a decoder $G$. The encoder maps a short temporal window of features to a continuous latent $z_e$, which is quantized to its nearest codebook entry $z_q = e_k$. Training minimizes a reconstruction loss together with the codebook-alignment and commitment terms of the standard VQ-VAE objective, with the codebook updated by exponential moving average for stability @oord_neural_2017. The use of a VQ-VAE to encode motion follows the discrete-latent line established for humanoid control by the NCP and MoConVQ frameworks which are adapted here to quadruped locomotion data. The hard partition imposed by the discrete codebook avoids the mode-averaging that a continuous latent exhibits when interpolating between qualitatively distinct gaits such as a walk and a gallop.

The central empirical question at this stage is whether the codebook organizes the MANN repertoire by gait without gait labels. This is assessed by encoding the full dataset and projecting the codebook assignments to lower dimensions using dimensionality reduction techniques (t-SNE, UMAP or PCA) then verifying visually that distinct gaits occupy distinct, separable regions of the latent space. A clean partition is direct evidence that the discrete representation supplies the skill structure that the later AMP conditioning stage requires.

// Generated artifacts. Two-dimensional projections of the encoded motion windows (via t-SNE, UMAP, or PCA), used to compare the unstructured motion sequences against their partitioned latent representation.

== Baseline Reinforcement Learning Framework

Before introducing the latent conditioning, the baseline motion-imitation algorithms must be established on the Go2 character. These are built on MimicKit @peng_mimickit_2026, an open-source framework that provides reference implementations of motion-imitation methods and RL algorithms, including DeepMimic and AMP, under a unified and modular training interface.

MimicKit ships with GPU-parallel simulator back ends (Isaac Gym and Isaac Lab) but does not provide a MuJoCo engine. To enable agile prototyping on a laptop CPU while retaining the option to scale to parallelized cloud training, this work extends MimicKit with a MuJoCo back end that exposes both standard CPU physics and GPU-accelerated simulation via MJX (MuJoCo XLA). This extension preserves MimicKit's existing environment, agent, and data abstractions while adding the flexibility to switch execution context between local CPU and cloud GPU.

// Generated artifact. A public fork of the MimicKit repository, extended with a MuJoCo/MJX backend for flexible CPU/GPU training.

== Latent-Conditioned AMP Training

The principal contribution of this work is the integration of the learned discrete motion tokens into the AMP training loop. With the VQ-VAE codebook frozen, the standard goal-conditioned policy $pi_theta (s_t, g_t)$ is modified to accept the discrete code as an explicit input, $pi_theta (s_t, g_t, k)$. An embedding layer maps each integer $k$ to its dense codebook vector before concatenation with $(Phi(s_t), g_t ; z_q)$. The discriminator $D_psi$ and the total reward $r_t = w^S \cdot r^S_t + w^G \cdot r^G_t$ are unchanged from vanilla AMP: the task goal $g_t$ dictates heading and speed, the discriminator supplies the style reward that keeps the motion dog-like, and the token $k$ acts as the skill-selection channel that determines which gait is used to satisfy the task.

The discrete bottleneck is expected to address the two open limitations of vanilla AMP identified in the Background. First, because each code addresses a separate region of the motion distribution, the policy is encouraged to retain distinct gaits rather than collapsing onto a single dominant one. Second, the codebook exposes an interpretable skill menu that a high-level controller can set directly, rather than coaxing behaviors indirectly through the task-reward channel alone. This stands in contrast to the closest quadruped precedent, @escontrela_adversarial_2022, which trains AMP on dog motion capture for a Unitree A1 but provides no latent structuring and therefore no explicit skill-selection channel.

// Generated artifacts. (1) Training and evaluation curves tracking imitation similarity and task performance. (2) Tabular comparisons of mode-collapse and skill-transition metrics against the vanilla AMP and DeepMimic baselines.

== Interactive Application

The final stage bridges simulation and interactive use by wrapping the trained latent-conditioned policy into a controllable system. A high-level finite state machine (FSM) serves as the user interface, translating user commands into the corresponding discrete codebook entries that select the active skill, which are then fed to the low-level policy alongside the task goal.

Deployment uses the Godot game engine, with a custom physics server backed by the MuJoCo simulation driving the control loop. This architecture follows @ranaweera_deep_2024, who demonstrated that Godot — through a Python communication framework over a socket interface and its flexible node-based scene system — is an effective and fully open-source platform for deep reinforcement learning and sim-to-real experimentation. Routing the trained policy through such an interface provides a richer control and presentation layer than a headless simulator, at no licensing cost.
