...
To demonstrate the practical viability of this approach for robotic control, successfully applied the AMP framework to a physical quadruped robot, the Unitree A1. Using a mere 4.5 seconds of motion capture data recorded from a real German Shepherd, their system learned a robust motion prior that completely substituted the need for complex, handcrafted reward functions. The resulting policies transferred effectively from simulation to the real world and yielded highly energy-efficient locomotion strategies. Notably, the policy demonstrated natural, autonomous gait transitions—such as shifting from a pace to a canter as the commanded velocity increased—validating that AMP can distill biologically efficient movement patterns from minimal reference data. Given its focus on adapting dog motion capture to a quadrupedal robotic morphology, this specific application represents the closest prior work to our proposed methodology.

// -- Latent structuring related works

The absence of an explicit skill structure in AMP motivated a line of work that augments adversarial training with a learned latent space, pursued along continuous and discrete tracks. On the continuous track, @peng_ase_2022 introduced Adversarial Skill Embeddings (ASE), extending AMP with a latent variable $z in RR^{64}$ that conditions the low-level policy; a mutual-information objective ensures that different codes yield distinguishable behaviors, and the resulting skill space can be reused across downstream tasks. @tessler_calm_2023 built CALM on the same continuous foundation, adding text or label conditioning so that behaviors can be directed by high-level descriptions. @serifi_vmp_2024 proposed Versatile Motion Priors (VMP), which trains a continuous VAE over short motion windows and conditions a tracking policy on the time-varying latent, achieving robust tracking of diverse and unseen motions. These methods share the smoothness of continuous latents and, with it, the mode-averaging tendency that blurs transitions between qualitatively distinct gaits @zhu_neural_2023.

On the discrete track, @zhu_neural_2023 introduced Neural Categorical Priors (NCP), which uses a VQ-VAE to compress motion into a discrete codebook and then trains a categorical prior over that codebook, with a prior-shifting procedure to prevent skill collapse and a high-level controller that selects entries for downstream tasks. NCP reported substantially improved motion quality and diversity over continuous baselines on humanoid characters. @yao_moconvq_2024 extended this direction with MoConVQ, which couples a VQ-VAE motion representation with model-based reinforcement learning to learn from tens of hours of unstructured data, yielding a token interface amenable to text-driven generation and integration with language models. Both NCP and MoConVQ were demonstrated exclusively on humanoid characters, and both retain DeepMimic-style tracking rewards during the representation-learning stage rather than an adversarial prior.

Recent work has demonstrated the efficacy of generative autoencoders in structuring complex, high-dimensional quadruped locomotion, as notably exemplified by @mitchell_gaitor_2023  in their Gaitor framework. By training a $beta$-VAE purely on expert demonstrations, the authors successfully map rich histories of raw robot states—including joint angles, velocities, and contact forces—into a highly compact 2D continuous latent space. Crucially, the regularizing pull of the VAE's loss formulation forces the encoded distributions of distinct gaits to overlap, creating a smooth representation that inherently captures the phase relationships between legs and enables fluid, interpolated transitions from a trot to a crawl. While this approach definitively proves that autoencoders can reliably extract and map raw kinematic data into a structured latent signal, Gaitor's reliance on forced smoothness to blend skills perfectly illustrates the limitations of continuous spaces for tasks requiring crisp skill execution. This establishes the direct architectural groundwork for your thesis by highlighting the necessity of replacing the continuous bottleneck with a discrete Vector Quantized (VQ) codebook to enforce unambiguous, unblurred gait selection.

Applied to motion, the VQ-VAE encoder ingests short temporal windows of kinematic features — root and body velocities, joint configurations, foot-contact patterns — and compresses each window into a discrete token $k$. Several properties make this attractive for quadruped locomotion. First, after training the codebook tends to partition the motion space by behavior without ever being given gait labels: distinct entries come to specialize in distinct gaits @zhu_neural_2023. Second, the finite codebook acts as a regularizer that discourages memorization of individual clips and encourages generalization. Third, a sequence of tokens is compositional, so a trajectory of skills can be represented as a string of codebook indices @yao_moconvq_2024.
